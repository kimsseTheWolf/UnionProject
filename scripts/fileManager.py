...
class basicFileManager():
    ...
    def createDescriptionFile(self, selectedCategory, selectedProject, fileName, whetherGUI:bool):
        if (whetherGUI == True):
...
class fileManager():
    ...
    def importFile(selectedCategory, selectedProject, localFileDirection):
        ...
        if (os.path.exists(localFileDirection) == True):
            ...
            if (os.path.exists(inprogram_location + file_name) == True):
                ...
                shutil.copyfile(localFileDirection, inprogram_location + file_name)
                ...
                basic_file_manager.createDescriptionFile(selectedCategory, selectedProject, file_name, False)
                print("Create Successfully! : " + file_name)
                pass
        else:
            print("This file does not exist!")
            
    def GUIimportFile(selectedCategory, selectedProject, localFileDirection):
        ...
        if (os.path.exists(localFileDirection) == True):
            ...
            if (os.path.exists(inprogram_location + file_name) == True):
                ...
                #         new_fileLocation = new_fileLocation + i
                print(new_fileLocation)
                shutil.copy(localFileDirection, inprogram_location + file_name)
                # generate single file's json properties.
                basic_file_manager.createFileJSONDescription(selectedCategory, selectedProject, file_name, localFileDirection)
                # add an discription file for this imported file
                basic_file_manager.createDescriptionFile(selectedCategory, selectedProject, file_name, True)
                print("Create Successfully! : " + file_name)
                return True
                pass
        else:
    ...
    def createFile (selectedCategory, selectedProject, fileName):
        inprogram_location = "./" + selectedCategory + "/" + selectedProject + "/"
        win_inprogram_location = ".\\" + selectedCategory + "\\" + selectedProject + "\\"
        ...
        try:
            os.mkdir(win_inprogram_location + fileName)
            file = open(win_inprogram_location + fileName + "\\" + fileName, "x")
            file.close()
            basic_file_manager.addListFileItem(selectedCategory, selectedProject, fileName)
            basic_file_manager.createFileJSONDescription(selectedCategory, selectedProject, fileName, "No origin file location")
            basic_file_manager.createDescriptionFile(selectedCategory, selectedProject, fileName, True)
            print("Create Successfully! : " + fileName)
            return True
        except:
            print("Failed")
            return False
